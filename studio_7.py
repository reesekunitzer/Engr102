...
def main():
    ...
    sorted_filtered_average_salary_by_industry = []

    for group in average_salaries_by_industry:
        if group.participant_count >= 10:
            sorted_filtered_average_salary_by_industry.append(group)

    sorted_filtered_average_salary_by_industry = sorted(sorted_filtered_average_salary_by_industry, key = lambda x: x.average, reverse=True)

    top_5_industries = []

    for entry in sorted_filtered_average_salary_by_industry[:5]:
        top_5_industries.append(entry.key)
    print(sorted_filtered_average_salary_by_industry[:5])
